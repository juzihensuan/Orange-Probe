...
interface AgentUpdateInfo {
  ...
  status: "available" | "current" | "manual" | "server-required" | "pending" | "installing" | "completed" | "failed";
  error: string;
  requestedAt: number;
  updatedAt: number;
...
export default function UpdateManagement() {
  ...
  const load = async (showFeedback = false) => {
    ...
    try {
      const response = await fetch(`/api/admin/updates${showFeedback ? "?refresh=1" : ""}`);
      ...
      if (showFeedback) {
        const text = payload.server.updateAvailable
          ...
          : `检查完成：服务端与 Agent 更新包均为 v${payload.agentPackageVersion}`;
  ...
  if (!data) return <div className="management-empty"><XCircle size={24} /><b>无法读取更新状态</b><button className="secondary-button" onClick={() => load(true)}>重试</button></div>;

  const availableAgents = data.agents.filter((agent) => agent.updateAvailable && !new Set(["pending", "installing"]).has(agent.status)).length;
  const serverUpdateActive = new Set(["requested", "running", "restarting"]).has(data.server.status);
