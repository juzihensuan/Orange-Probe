...
export default function UpdateManagement() {
  ...
  const availableAgents = data.agents.filter((agent) => agent.updateAvailable).length;
  return <div className="page-stack update-management">
    ...
    <section className="update-server-panel">
      <div className="update-panel-heading"><span><Server size={18} /></span><div><b>服务端</b><small>由内部更新容器拉取 GHCR 镜像并保留数据卷重建</small></div><a href={data.server.releaseUrl} target="_blank" rel="noreferrer">Release <ExternalLink size={13} /></a></div>
      ...
      {!data.server.updaterConfigured && <p className="update-inline-warning"><AlertTriangle size={14} />当前部署没有连接更新容器，请使用 v1.1.2 的 Docker Compose 或一键安装命令部署。</p>}
