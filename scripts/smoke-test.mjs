...
import { dateParts, trafficCounterDelta, trafficWindowForConfig } from "../server/traffic.js";
...
const childLogs = [];
let mockReleaseVersion = "1.2.4";
let probeProcess;
let agentProcess;
let updaterProcess;

function testTrafficAccountingRules() {
  assert.equal(dateParts("2026-02-31"), null, "Invalid calendar dates must be rejected");

  const monthly = trafficWindowForConfig(
    { billingCycle: "monthly", purchaseDate: "2026-01-21", expirationDate: "2028-08-12" },
    Date.UTC(2026, 6, 28, 12),
  );
  assert.equal(monthly.start, Date.UTC(2026, 6, 12));
  assert.equal(monthly.end, Date.UTC(2026, 7, 12));
  assert.equal(monthly.anchorSource, "expirationDate");

  const annual = trafficWindowForConfig(
    { billingCycle: "annual", purchaseDate: "2026-01-21", expirationDate: "2029-01-09" },
    Date.UTC(2026, 6, 28, 12),
  );
  assert.equal(annual.start, Date.UTC(2026, 6, 21));
  assert.equal(annual.end, Date.UTC(2026, 7, 21));
  assert.equal(annual.anchorSource, "purchaseDate");

  const normal = trafficCounterDelta(
    { download: 610, upload: 205 },
    { download: 600, upload: 200 },
    "linux-route-v2:eth0",
    "linux-route-v2:eth0",
  );
  assert.deepEqual(normal, { download: 10, upload: 5, rebased: false });
  const rollback = trafficCounterDelta(
    { download: 540, upload: 180 },
    { download: 610, upload: 205 },
    "linux-route-v2:eth0",
    "linux-route-v2:eth0",
  );
  assert.deepEqual(rollback, { download: 0, upload: 0, rebased: true });
  const interfaceChange = trafficCounterDelta(
    { download: 900, upload: 300 },
    { download: 540, upload: 180 },
    "linux-route-v2:eth1",
    "linux-route-v2:eth0",
  );
  assert.deepEqual(interfaceChange, { download: 0, upload: 0, rebased: true });
}
...
async function testAgentUpdaterTransaction() {
  ...
  try {
    ...
    const updater = spawn(process.execPath, ["agent/updater.js", "--staging", stagingDir, "--install", installDir, "--data", dataDir, "--parent", String(legacyParent.pid), "--version", "1.2.2", "--attempt", attemptId], {
      cwd: rootDir,
    ...
    assert.equal(JSON.parse(fs.readFileSync(path.join(installDir, "package.json"), "utf8")).version, "1.2.2");
    const result = JSON.parse(fs.readFileSync(path.join(dataDir, "update-result.json"), "utf8"));
    assert.deepEqual({ state: result.state, targetVersion: result.targetVersion, attemptId: result.attemptId }, { state: "success", targetVersion: "1.2.2", attemptId });
    assert.equal(fs.existsSync(stagingDir), false);
    assert.equal(JSON.parse(fs.readFileSync(path.join(dataDir, "update-backup", "package.json"), "utf8")).version, "1.2.1");

    const directStagingDir = path.join(dataDir, "update-direct");
    fs.mkdirSync(directStagingDir, { recursive: true });
    const directFiles = { "index.js": "// direct Agent\n", "region.js": "// direct region\n", "updater.js": "// direct updater\n", "package.json": packageJson("1.2.3"), "package-lock.json": packageLock("1.2.3") };
    ...
    assert.equal(JSON.parse(fs.readFileSync(path.join(dataDir, "update-backup", "package.json"), "utf8")).version, "1.2.2");
  } finally {
...
async function testAgentSelfUpdateEndToEnd() {
  ...
  try {
    ...
    for (const file of ["index.js", "region.js", "updater.js", "package.json", "package-lock.json"]) {
      ...
      if (file === "index.js") content = Buffer.from(content.toString("utf8").replace('version: "1.2.3"', 'version: "1.2.4"'));
      if (file === "package.json" || file === "package-lock.json") content = Buffer.from(content.toString("utf8").replaceAll('"version": "1.2.3"', '"version": "1.2.4"'));
    ...
    updateServer = http.createServer((request, response) => {
      if (request.method === "GET" && request.url === "/manifest.json") {
        ...
        response.end(JSON.stringify({ version: "1.2.4", files: Object.entries(releaseFiles).map(([name, file]) => ({ name, url: `/files/${name}`, sha256: file.sha256 })) }));
        return;
      }
      ...
      if (request.method === "POST" && request.url === "/api/agents/report") {
        ...
        request.on("end", () => {
          ...
          if (!dispatched && report.version === "1.2.3") {
            dispatched = true;
            responseBody.update = { version: "1.2.4", manifestUrl: "/manifest.json", attemptId };
          }
          if (report.version === "1.2.4" && report.updateStatus?.state === "success") responseBody.updateStatusAcknowledged = true;
          response.writeHead(200, { "content-type": "application/json" });
    ...
    assert.equal(JSON.parse(fs.readFileSync(path.join(installDir, "package.json"), "utf8")).version, "1.2.4");
    ...
    await waitFor(() => reports.some((report) => report.version === "1.2.4" && report.updateStatus?.state === "success"), "Restarted Agent did not confirm v1.2.4", 15_000);
...
function startProbe() {
  const child = spawn(process.execPath, ["server/index.js"], {
    ...
    env: {
      ...
      TRUST_PROXY: "loopback, linklocal, uniquelocal, 198.51.100.0/24",
    },
    stdio: ["ignore", "pipe", "pipe"],
  });
  child.stdout.on("data", (chunk) => childLogs.push(String(chunk)));
...
function blockedWebSocket(ip, pathname = "/ws") {
  return new Promise((resolve, reject) => {
    const socket = new WebSocket(`ws://127.0.0.1:${probePort}${pathname}`, { origin, headers: { "x-forwarded-for": ip } });
    let settled = false;
    const finish = (error) => {
      if (settled) return;
      settled = true;
...
try {
  ...
  await request("/api/admin/login", {
    ...
    expectedStatus: 401,
    headers: {
      origin: "http://probe-preview.example:5173",
      "x-forwarded-host": "probe-preview.example:5173",
      "x-forwarded-for": "203.0.113.59",
    },
    body: { username: adminUsername, password: "wrong-password" },
  });
  await login();

  const trafficToken = crypto.randomUUID();
  const { payload: trafficAgent } = await request("/api/admin/agents", {
    method: "POST",
    body: { name: "Traffic Counter Regression", token: trafficToken },
  });
  await request(`/api/admin/servers/${trafficAgent.id}`, {
    method: "PUT",
    body: {
      name: "Traffic Counter Regression",
      billingCycle: "annual",
  ...
  const reportTraffic = (downloadGiB, uploadGiB, counterKey = "linux-route-v2:eth0") => request("/api/agents/report", {
    method: "POST",
    authenticated: false,
    headers: { "x-probe-token": trafficToken },
    body: {
      ...agentPayload("Traffic Counter Regression"),
      network: {
        downloadSpeed: 0,
        uploadSpeed: 0,
        downloadTotal: downloadGiB * 1024 ** 3,
        uploadTotal: uploadGiB * 1024 ** 3,
        counterKey,
      },
    },
  });
  await reportTraffic(600, 200);
  await reportTraffic(610, 205);
  let { payload: trafficServers } = await request("/api/admin/servers");
  let trafficNode = trafficServers.servers.find((server) => server.id === trafficAgent.id);
  assert.equal(trafficNode.trafficUsed, 15 * 1024 ** 3);
  assert.equal(trafficNode.trafficResetDay, 21);
  assert.equal(trafficNode.trafficResetSource, "purchaseDate");

  await reportTraffic(540, 180);
  ({ payload: trafficServers } = await request("/api/admin/servers"));
  trafficNode = trafficServers.servers.find((server) => server.id === trafficAgent.id);
  assert.equal(trafficNode.trafficUsed, 15 * 1024 ** 3, "A raw counter rollback was incorrectly added as new traffic");
  await reportTraffic(545, 183);
  await reportTraffic(900, 300, "linux-route-v2:eth1");
  ({ payload: trafficServers } = await request("/api/admin/servers"));
  trafficNode = trafficServers.servers.find((server) => server.id === trafficAgent.id);
  assert.equal(trafficNode.trafficUsed, 23 * 1024 ** 3, "An interface change was incorrectly added as new traffic");
  await new Promise((resolve) => setTimeout(resolve, 1200));
  const persistedTraffic = JSON.parse(fs.readFileSync(path.join(dataDir, "traffic-totals.json"), "utf8"));
  assert.equal(persistedTraffic[trafficAgent.id].windowDownload + persistedTraffic[trafficAgent.id].windowUpload, 23 * 1024 ** 3);
  ...
  await request(`/api/admin/servers/${trafficAgent.id}`, {
    ...
    body: {
      ...
      purchaseDate: "2026-01-21",
      expirationDate: "2029-01-12",
    },
  });
  await reportTraffic(905, 303, "linux-route-v2:eth1");
  ({ payload: trafficServers } = await request("/api/admin/servers"));
  trafficNode = trafficServers.servers.find((server) => server.id === trafficAgent.id);
  assert.equal(trafficNode.trafficUsed, 0, "Traffic from the prior reset window leaked into the new window");
  assert.equal(trafficNode.trafficResetDay, 12);
  assert.equal(trafficNode.trafficResetSource, "expirationDate");
  await reportTraffic(906, 304, "linux-route-v2:eth1");
  ({ payload: trafficServers } = await request("/api/admin/servers"));
  trafficNode = trafficServers.servers.find((server) => server.id === trafficAgent.id);
  assert.equal(trafficNode.trafficUsed, 2 * 1024 ** 3);

  await request(`/api/admin/servers/${trafficAgent.id}`, {
    method: "PUT",
    expectedStatus: 400,
    body: { expirationDate: "2026-02-31" },
  });
  await request(`/api/admin/servers/${trafficAgent.id}`, { method: "DELETE" });
  ...
  assert.equal(initialFirewall.currentIpSource, "socket");
  assert.deepEqual(initialFirewall.blocked, []);
  const { payload: realIpFallback } = await request("/api/admin/firewall", { headers: { "x-real-ip": "203.0.113.60" } });
  assert.equal(realIpFallback.currentIp, "203.0.113.60");
  assert.equal(realIpFallback.currentIpSource, "x-real-ip");
  const { payload: trustedProxyChain } = await request("/api/admin/firewall", { headers: { "x-forwarded-for": "203.0.113.61, 198.51.100.20", "x-real-ip": "192.0.2.200" } });
  assert.equal(trustedProxyChain.currentIp, "203.0.113.61");
  assert.equal(trustedProxyChain.currentIpSource, "x-forwarded-for");
  const { payload: restoredVisitorIp } = await request("/api/admin/firewall", { headers: { "x-forwarded-for": "203.0.113.63, 173.245.48.10", "cf-connecting-ip": "203.0.113.63" } });
  assert.equal(restoredVisitorIp.currentIp, "203.0.113.63");
  assert.equal(restoredVisitorIp.currentIpSource, "visitor-header");
  const { payload: spoofedVisitorIp } = await request("/api/admin/firewall", { headers: { "x-forwarded-for": "192.0.2.21", "cf-connecting-ip": "203.0.113.250" } });
  assert.equal(spoofedVisitorIp.currentIp, "192.0.2.21", "A visitor spoofed the final visitor IP header");
  const { payload: untrustedProxyChain } = await request("/api/admin/firewall", { headers: { "x-forwarded-for": "203.0.113.62, 192.0.2.20" } });
  assert.equal(untrustedProxyChain.currentIp, "192.0.2.20", "An untrusted intermediary was incorrectly skipped");
  ...
  for (const pathname of ["/", "/admin", "/server/smoke-node"]) {
    const { response, payload } = await request(pathname, { authenticated: false, expectedStatus: 403, headers: { accept: "text/html", "x-forwarded-for": manualBlockedIp } });
    assert.match(payload.text, /UFW 提示/);
    assert.match(payload.text, /你的 IP 已被封禁/);
    assert.match(payload.text, new RegExp(manualBlockedIp.replaceAll(".", "\\.")));
    assert.match(String(response.headers.get("cache-control")), /no-store/);
  }
  for (const pathname of ["/api/health", "/downloads/agent/index.js", "/flags/us.svg"]) {
    const { payload } = await request(pathname, { authenticated: false, expectedStatus: 403, headers: { "x-forwarded-for": manualBlockedIp } });
    assert.equal(payload.ip, manualBlockedIp);
    assert.equal(payload.blocked, true);
  }
  await blockedWebSocket(manualBlockedIp);
  await blockedWebSocket(manualBlockedIp, "/agent-ws");
  ...
  assert.equal(automaticBlock.remainingAttempts, 0);
  const adminLoginSource = fs.readFileSync(path.join(rootDir, "src", "AdminLogin.tsx"), "utf8");
  assert.match(adminLoginSource, /remainingAttempts/);
  assert.doesNotMatch(adminLoginSource, /连续失败 5 次将封禁当前 IP/);
  const firewallClientSource = fs.readFileSync(path.join(rootDir, "src", "firewall.ts"), "utf8");
  assert.match(firewallClientSource, /document\.body\.replaceChildren/);
  ...
  assert.equal(agentManifest.version, "1.2.3");
  ...
  assert.ok(agentManifest.files.every((file) => file.url.endsWith("?v=1.2.3")));
  ...
  assert.match(serverUpdaterSource, /resolveVersion\(targetVersion\)/);
  assert.match(serverUpdaterSource, /timingSafeEqual/);
  ...
  assert.match(serverUpdaterSource, /registry authorization failed/);
  assert.match(serverUpdaterSource, /docker", \["login", registry/);
  assert.match(serverUpdaterSource, /configuredImages\(\)/);
  assert.doesNotMatch(serverUpdaterSource, /releaseComposeFile/);
  assert.doesNotMatch(serverUpdaterSource, /docker", \["build"/);
  assert.doesNotMatch(serverUpdaterSource, /Orange-Probe-Docker/);
  assert.match(serverUpdaterSource, /--volumes-from/);
  assert.match(serverUpdaterSource, /orange-probe-updater/);
  assert.match(serverUpdaterSource, /--force-recreate/);
  assert.match(serverUpdaterSource, /restartScope: "containers-only"/);
  assert.doesNotMatch(serverUpdaterSource, /\b(?:reboot|shutdown)\b|systemctl\s+reboot/i);
  ...
  assert.equal(initialUpdates.server.currentVersion, "1.2.3");
  assert.equal(initialUpdates.server.latestVersion, "1.2.4");
  assert.equal(initialUpdates.agentPackageVersion, "1.2.3");
  ...
  assert.equal(serverUpdate.targetVersion, "1.2.4");
  assert.equal(serverUpdate.restartScope, "containers-only");
  ...
  assert.equal(liveAgentUpdate.version, "1.2.3");
  ...
  assert.equal(blockedAgentUpdate.targetVersion, "1.2.4");
  ...
  assert.equal(refreshableAgent.targetVersion, "1.2.3");
  ...
  assert.equal(updateInstruction.update.version, "1.2.3");
  ...
  const { payload: completedInstruction } = await request("/api/agents/report", { method: "POST", authenticated: false, headers: { "x-probe-token": updateAgentToken }, body: { ...oldUpdatePayload, version: "1.2.3", updateStatus: { state: "success", targetVersion: "1.2.3", attemptId: updateInstruction.update.attemptId } } });
  ...
  assert.match(updateManagementSource, /savePendingServerRefresh/);
  assert.match(updateManagementSource, /宿主机/);
  const applicationSource = fs.readFileSync(path.join(rootDir, "src", "App.tsx"), "utf8");
  assert.match(applicationSource, /readPendingServerRefresh/);
  assert.match(applicationSource, /window\.location\.reload\(\)/);
  assert.match(applicationSource, /\/api\/health/);
  ...
  assert.match(agentSource, /linuxDefaultRouteInterfaces/);
  assert.match(agentSource, /linux-\$\{selected\.mode\}-v2/);
  ...
  assert.equal(timeoutInstruction.update.version, "1.2.3");
  await new Promise((resolve) => setTimeout(resolve, 1200));
  ...
  const { payload: defaultPortService } = await request("/api/admin/services", {
    method: "POST",
    body: { name: "Smoke Default Port TCP", type: "tcp", target: "127.0.0.1", interval: 5, serverIds: [agent.id], enabled: false },
  });
  assert.equal(defaultPortService.target, "127.0.0.1");
  await request(`/api/admin/services/${defaultPortService.id}`, { method: "DELETE" });
  const { payload: secondaryService } = await request("/api/admin/services", {
    method: "POST",
    body: { name: "Smoke TCP Monitor", type: "tcp", target: `127.0.0.1:${mockPort}`, interval: 5, serverIds: [agent.id], enabled: true },
  });
  ...
  const tcpHistory = await waitFor(async () => {
    const { payload } = await request(`/api/services/${secondaryService.id}/history?serverId=${agent.id}&period=realtime`);
    return payload.points.find((point) => point.source === "agent" && point.success) ? payload : null;
  }, "Agent TCPing result was not returned", 15_000, 500);
  const tcpPoint = tcpHistory.points.filter((point) => point.success && point.latency !== null).at(-1);
  assert.ok(tcpPoint.latency < 250, `Local TCPing latency was unexpectedly high (${tcpPoint.latency} ms)`);
  ...
  const agentMonitorSource = fs.readFileSync(path.join(rootDir, "agent", "index.js"), "utf8");
  assert.match(agentMonitorSource, /dns\.lookup/);
  assert.match(agentMonitorSource, /highLatencyThresholdMs = 1000/);
  assert.match(agentMonitorSource, /highLatencyRetries = 3/);
  assert.match(agentMonitorSource, /tcpRetransmissionDropMs = 800/);
  assert.match(agentMonitorSource, /Suspicious TCP handshake retransmission detected/);
  assert.doesNotMatch(agentMonitorSource, /tcpPingSamples|latencies\.sort/);
  const formatSource = fs.readFileSync(path.join(rootDir, "src", "lib", "format.ts"), "utf8");
  assert.match(formatSource, /stableLatencyValues/);
