...
async function testAgentUpdaterTransaction() {
  ...
  try {
    ...
    const updater = spawn(process.execPath, ["agent/updater.js", "--staging", stagingDir, "--install", installDir, "--data", dataDir, "--parent", String(legacyParent.pid), "--version", "1.1.7", "--attempt", attemptId], {
      cwd: rootDir,
    ...
    assert.deepEqual({ state: result.state, targetVersion: result.targetVersion, attemptId: result.attemptId }, { state: "success", targetVersion: "1.1.7", attemptId });
    ...
    assert.equal(installed.version, "1.1.8");
    assert.equal(installed.dependenciesChanged, false);
    assert.equal(JSON.parse(fs.readFileSync(path.join(dataDir, "update-backup", "package.json"), "utf8")).version, "1.1.7");
  } finally {
...
async function testAgentSelfUpdateEndToEnd() {
  ...
  try {
    ...
    for (const file of ["index.js", "region.js", "updater.js", "package.json", "package-lock.json"]) {
      ...
      if (file === "index.js") content = Buffer.from(content.toString("utf8").replace('version: "1.1.7"', 'version: "1.1.8"'));
      if (file === "package.json" || file === "package-lock.json") content = Buffer.from(content.toString("utf8").replaceAll('"version": "1.1.7"', '"version": "1.1.8"'));
    ...
    updateServer = http.createServer((request, response) => {
      if (request.method === "GET" && request.url === "/manifest.json") {
        ...
        response.end(JSON.stringify({ version: "1.1.8", files: Object.entries(releaseFiles).map(([name, file]) => ({ name, url: `/files/${name}`, sha256: file.sha256 })) }));
        return;
      }
      ...
      if (request.method === "POST" && request.url === "/api/agents/report") {
        ...
        request.on("end", () => {
          ...
          if (!dispatched && report.version === "1.1.7") {
            dispatched = true;
            responseBody.update = { version: "1.1.8", manifestUrl: "/manifest.json", attemptId };
          }
          if (report.version === "1.1.8" && report.updateStatus?.state === "success") responseBody.updateStatusAcknowledged = true;
          response.writeHead(200, { "content-type": "application/json" });
    ...
    await waitFor(() => reports.some((report) => report.version === "1.1.8" && report.updateStatus?.state === "success"), "Restarted Agent did not confirm v1.1.8", 15_000);
...
try {
  ...
  assert.equal(agentManifest.version, "1.1.7");
  ...
  assert.ok(agentManifest.files.every((file) => file.url.endsWith("?v=1.1.7")));
  ...
  assert.equal(initialUpdates.server.currentVersion, "1.1.7");
  assert.equal(initialUpdates.server.latestVersion, "1.1.8");
  assert.equal(initialUpdates.agentPackageVersion, "1.1.7");
  ...
  assert.equal(serverUpdate.targetVersion, "1.1.8");
  ...
  assert.equal(liveAgentUpdate.version, "1.1.7");
  ...
  assert.equal(blockedAgentUpdate.targetVersion, "1.1.8");
  ...
  assert.equal(refreshableAgent.targetVersion, "1.1.7");
  ...
  assert.equal(updateInstruction.update.version, "1.1.7");
  ...
  const { payload: completedInstruction } = await request("/api/agents/report", { method: "POST", authenticated: false, headers: { "x-probe-token": updateAgentToken }, body: { ...oldUpdatePayload, version: "1.1.7", updateStatus: { state: "success", targetVersion: "1.1.7", attemptId: updateInstruction.update.attemptId } } });
  ...
  assert.equal(timeoutInstruction.update.version, "1.1.7");
